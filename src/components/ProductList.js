...
import { selectModal, selectModalDel, setStateModal, setStateModalDel } from '../store/popup';
import { selectProductCart, selectProductFavorites, setCart, setFavorites } from '../store/productList';
...
const ProductList = connect(mapStateToProps, mapDispatchToProps)(({items, favorites, setFavorites, cart, setCart, delBtnCart, stateModal, setStateModal, stateModalDel, setStateModalDel}) => {    
    ...
    const listItems = items.map( (item) => 
        <li key={item.id} data-id={item.id} className="product-list-item">
            ...
            {delBtnCart && <FontAwesomeIcon icon={ faTrashAlt } onClick={delCartHandler} />}
        </li> )
